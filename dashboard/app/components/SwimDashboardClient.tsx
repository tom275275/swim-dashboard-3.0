...
import type { Schedule, SwimEvent } from '../page'
...
function formatDateTab(dateStr: string, todayStr: string): string {
...
export default function SwimDashboardClient({ schedule }: { schedule: Schedule }) {
  ...
  const filteredEvents = schedule.events
    .filter((e) => {
      ...
      return mode === 'family' ? isFamilyActivity(e.activity) : !isFamilyActivity(e.activity)
    })
        ...
        <>
          ...
          {isToday && hasHighlighted && (
            <section className="mb-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                {activeNow.map((e) => (
                  <EventCard key={`${e.date}-${e.center}-${e.start_time}`} event={e} highlight="now" address={schedule.centres?.[e.center]?.address} />
                ))}
                {startingSoon.map((e) => (
                  <EventCard key={`${e.date}-${e.center}-${e.start_time}`} event={e} highlight="soon" address={schedule.centres?.[e.center]?.address} />
                ))}
              </div>
            </section>
          )}
          ...
          {laterEvents.length > 0 && (
            <section>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                {laterEvents.map((e) => (
                  <EventCard key={`${e.date}-${e.center}-${e.start_time}`} event={e} address={schedule.centres?.[e.center]?.address} />
                ))}
              </div>
            </section>
          )}
